    ...
    for x in range(xmin, xmax):
        for y in range(ymin, ymax):
            ...
            if(coords[0] >=0 and coords[1] >= 0 and coords[2] >= 0):
                ...
                if(z_ < Z_BUFFER[y, x]):
                    Z_BUFFER[y, x] = z_
                    mat[y, x] = -255*(coords[0]*color[0] + coords[1]*color[1] + coords[2]*color[2])
...
#----------------------------------------------------------------
def create_image(H, W, filename, v, f, vn, vt):
    ...
    for fi in range(len(f)):
        ...
        y2 = v[f[fi][2] - 1][1]
        z2 = v[f[fi][2] - 1][2]
        vn_calc[f[fi][0] - 1] += normal(x0, y0, z0, x1, y1, z1, x2, y2, z2)
        vn_calc[f[fi][1] - 1] += normal(x0, y0, z0, x1, y1, z1, x2, y2, z2)
        vn_calc[f[fi][2] - 1] += normal(x0, y0, z0, x1, y1, z1, x2, y2, z2)
    ...
    for fi in range(len(f)):
        ...
        cosB = Ii[f[fi][1] - 1]
        cosC = Ii[f[fi][2] - 1]
        draw_triangle(image_array, x0, y0, z0, x1, y1, z1, x2, y2, z2, (cosA, cosB, cosC))
...
#---------------------------------------
create_image(WIDTH, HEIGHT, 'image_z_buffer.png', v, f0, vn, vt)